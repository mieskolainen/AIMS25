...
def searchsorted(bin_locations, inputs, eps=1e-6):
    """Locate spline bins, shifting the final edge in place by eps

    Args:
        bin_locations (Tensor): Sorted edges (..., K + 1)
        inputs (Tensor): Values (...)
        eps (float): Offset for the final edge
...
def unconstrained_RQS(inputs, unnormalized_widths, unnormalized_heights,
    ...
    """Apply a rational-quadratic spline with identity tails

    Args:
        inputs (Tensor): Coordinates (...)
        unnormalized_widths (Tensor): Width logits (..., K)
        unnormalized_heights (Tensor): Height logits (..., K)
        unnormalized_derivatives (Tensor): Interior derivative logits (..., K - 1)
        inverse (bool): Apply the inverse spline
        tail_bound (float): Spline domain half-width
        min_bin_width (float): Minimum normalized bin width
        min_bin_height (float): Minimum normalized bin height
        min_derivative (float): Minimum knot derivative

    Returns:
        tuple[Tensor, Tensor]: Transformed values and log Jacobians, each (...)
    """
    ...
    if not torch.any(inside_intvl_mask):
...
def RQS(inputs, unnormalized_widths, unnormalized_heights,
        ...
        min_bin_height=DEFAULT_MIN_BIN_HEIGHT,
        min_derivative=DEFAULT_MIN_DERIVATIVE):
    """Apply a bounded rational-quadratic spline

    Args:
        inputs (Tensor): Coordinates (...)
        unnormalized_widths (Tensor): Width logits (..., K)
        unnormalized_heights (Tensor): Height logits (..., K)
        unnormalized_derivatives (Tensor): Knot derivative logits (..., K + 1)
        inverse (bool): Apply the inverse spline
        left (float): Input lower bound
        right (float): Input upper bound
        bottom (float): Output lower bound
        top (float): Output upper bound
        min_bin_width (float): Minimum normalized bin width
        min_bin_height (float): Minimum normalized bin height
        min_derivative (float): Minimum knot derivative
    ...
    lower, upper = (bottom, top) if inverse else (left, right)
    if torch.min(inputs) < lower or torch.max(inputs) > upper:
