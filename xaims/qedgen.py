...
def bhabha_amplitude2_massless(s: float, t: np.ndarray):
    ...
    """
    u = - s - t    
    return 2*e_qed**4 * ((t**2 + u**2) / (s**2) + (s**2 + u**2) / (t**2) + (2*u**2) / (s*t))
...
def bhabha_amplitude2(s: float, t: np.ndarray, m=5.11e-4):
    ...
    return 2*e_qed**4*(num_s/(s*s) + num_t/(t*t) + num_int/(s*t))
...
def moeller_amplitude2(s: float, t: np.ndarray, m=5.11e-4):
    ...
    num_t = (s - 2*m**2)**2 + (u - 2*m**2)**2
    num_u = (s - 2*m**2)**2 + (t - 2*m**2)**2
    num_int = -(s - 2*m**2)**2
    
    return 2*e_qed**4*(num_t/(t*t) + num_u/(u*u) + num_int/(t*u))
...
def moeller_amplitude2_u(s: float, t: np.ndarray, m=5.11e-4):
    ...
    u = 4*m**2 - s - t
    num_u = (s - 2*m**2)**2 + (t - 2*m**2)**2
    return 2*e_qed**4*(num_u/(u*u))

@numba.njit
def moeller_amplitude2_int(s: float, t: np.ndarray, m=5.11e-4):
    """
    (inteference term only -- for diagnostics)
    """
    u = 4*m**2 - s - t
    num_int = -(s - 2*m**2)**2
    return 2*e_qed**4*(num_int/(t*u))
...
def alpha_eff(Q2):
    ...
    return 1.0/(137.035999 - 0.027*np.log(Q2))
...
def generate_event(s: float, t_min: float, t_max: float, theory: Mode):
    ...
    w_max = V * kinematics(s) * amplitude2_wrapper(s=s, t=t_max, theory=theory)
