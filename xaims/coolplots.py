...
def scattering_potential_qed(
    ...
    """Plot classical Coulomb trajectories colored by QED squared amplitudes

    Args:
        m (float): Classical particle mass
        b0 (np.ndarray): Impact parameters (N,)
        vz0 (float): Initial longitudinal velocity
        vy0 (float): Initial transverse velocity
        potential_k (float | None): Coulomb strength, None overlays -1 and +1
        total_time (float): Trajectory duration
        sqrts (float): CM energy in GeV for QED amplitudes
        fontsize (int): Label font size
        cmap (Colormap): Amplitude color map
        facecolor (str): Background color
        labelcolor (str): Label color
        spinecolor (str): Axis border color

    Returns:
        tuple[Figure, Axes]: Displayed scattering plot
    """
    ...
    def deriv(state, k, m, eps=1e-8):
        """Evaluate softened Coulomb equations of motion

        Args:
            state (np.ndarray): State (4,) as z, y, vz, vy
            k (float): Coulomb strength
            m (float): Particle mass
            eps (float): Radial softening scale

        Returns:
            np.ndarray: State time derivative (4,)
        """
    ...
    def rk4_step(state, dt, k, m, eps=1e-8):
        """Advance a Coulomb trajectory by one fourth-order Runge-Kutta step
    ...
    def simulate_trajectory(b, k, m, total_time, dt=0.02, eps=1e-8):
        ...
        """Integrate a classical Coulomb scattering trajectory
        ...
        Returns:
            tuple[np.ndarray, float]: Positions (int(total_time / dt), 2) and scattering angle
        """
...
def plot_mandelstam(m=0.5):
    """Plot physical scattering regions in the Mandelstam plane

    Args:
        m (float): Common particle mass

    Returns:
        tuple[Figure, Axes]: Displayed Mandelstam diagram
    """
...
def OU_simulation(alpha=3.0, sigma=2.5, N=25, T=1.0, mu0=4.0, std0=0.75, plot_title=True, plot_hist=False):
    """Visualize Ornstein-Uhlenbeck diffusion and associated ODE trajectories

    Args:
        alpha (float): Mean-reversion rate
        sigma (float): Diffusion scale
        N (int): SDE trajectory count
        T (float): Terminal time
        mu0 (float): Initial Gaussian-mixture means at +/- mu0
        std0 (float): Initial component standard deviation
        plot_title (bool): Show the ODE title
        plot_hist (bool): Draw marginal histograms instead of analytic densities

    Returns:
        tuple: Displayed Figure and (forward Axes, reverse Axes)
    """
    ...
    def pdf_data(x, mean, std):
        """Evaluate a symmetric two-Gaussian mixture density

        Args:
            x (float | np.ndarray): Coordinates (...)
            mean (float): Component means at +/- mean
            std (float): Component standard deviation

        Returns:
            float | np.ndarray: Density matching x
        """
    ...
    def m(t):
        """Evaluate the positive Gaussian-component mean over time

        Args:
            t (float | np.ndarray): Times (...)

        Returns:
            float | np.ndarray: Component means matching t
        """

        return mu0 * np.exp(-alpha*t)
    def v(t):
        """Evaluate Gaussian-component variance over time

        Args:
            t (float | np.ndarray): Times (...)

        Returns:
            float | np.ndarray: Variances matching t
        """

        return v0 * np.exp(-2*alpha*t) + (sigma2/(2*alpha))*(1 - np.exp(-2*alpha*t))

    def score(x, t):
        """Evaluate the symmetric mixture score
        ...
        Returns:
            float | np.ndarray: Log-density derivatives matching x
        """
    ...
    # ODE
    def forward_ode(x, t):
        """Evaluate the forward probability-flow ODE drift

        Args:
            x (np.ndarray): States (N,)
            t (float): Forward time

        Returns:
            np.ndarray: State derivatives (N,)
        """

        return -alpha*x - 0.5*sigma2*score(x, t)

    def reverse_ode(x, s):
        """Evaluate the illustrated ODE with reversed time

        Args:
            x (np.ndarray): States (N,)
            s (float): Reverse time, t=T-s
    ...
    for i in range(n_steps):
        ...
        # Condition the earlier-time component on the current observation
        previous_mean = m_rev[i + 1]
        previous_var = v_rev[i + 1]
        cov = kappa * previous_var
        gain = cov / vt
        std = np.sqrt(previous_var * var_noise / vt)

        mu1 = previous_mean + gain * (xt - mt)
        mu2 = -previous_mean + gain * (xt + mt)
    ...
    for ax in (ax_f, ax_r):
        ...
        ax.set_ylim(-6, 6)
        ax.axvspan(T - dt/2, T + dt/2, color='red', alpha=0.15)
    ...
    if plot_title:
        ...
        ax_f.set_title(
            r'Forward SDE ($dt>0$)' + '\n'
            + r'$dx = -\alpha x\,dt + \sigma\,dw_t$', color='white')
        ax_r.set_title(
            r'Reverse SDE ($dt<0$)' + '\n'
            + r'$dx = (-\alpha x - \sigma^2 \nabla_x \log p_t(x))\,dt'
            + r' + \sigma\,d\bar{w}_t$', color='white')
