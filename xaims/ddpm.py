...
class DDPM(nn.Module):
    def __init__(self,
        ...
        """Build a discrete diffusion model with a linear noise schedule

        Args:
            x_dim (int): Data width D
            cond_dim (int | None): Condition width C
            nnet (nn.Module): Predict noise from data, time embedding, and conditions
            beta_start (float): Initial noise variance
            beta_end (float): Final noise variance
            diffusion_steps (int): Diffusion step count
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor=None):
        """Predict noise at normalized diffusion times

        Args:
            x (Tensor): Noisy data (B, D)
            t (Tensor): Normalized times (B, 1)
            cond (Tensor | None): Conditions (B, C)

        Returns:
            Tensor: Predicted noise (B, D)
        """
    ...
    def loss_wrapper(self, x: torch.Tensor, cond: torch.Tensor=None):
        """Draw noisy training inputs and predict their noise
        ...
        Returns:
            tuple[Tensor, Tensor]: Predicted and sampled noise, each (B, D)
        """
    ...
    def loss(self, x: torch.Tensor, cond: torch.Tensor=None):
        """Compute squared noise error summed over features
    ...
    def sample(self, num_samples: int, cond: torch.Tensor=None):
        """Draw samples by reversing the discrete diffusion process
        ...
        Returns:
            Tensor: Generated samples (S, D)
        """

        device = self.betas.device
        x = torch.randn(num_samples, self.x_dim, device=device, dtype=self.betas.dtype)

        for t in reversed(range(self.diffusion_steps)):

            t_norm     = x.new_full((num_samples, 1), t / self.diffusion_steps)
